...
def test_post_new_pet_with_valid_data(name="Tiki", animal_type="dog",
...
def test_update_pet_info_successfully(name="Snow", animal_type="dog", age="2"):
    _, auth_key = pf.get_api_key(valid_email, valid_password)
...
def test_delete_pet_successfully():
    ...
    if len(my_pets['pets']) == 0:
        pf.post_new_pet(auth_key, "Суперкот", "кот", "1", "images/cat.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
